...
inventoryList.DisplayController.prototype =
{
  ...
  initialize: function( inventoryListContainerId,url,initPagingValue,initPageIndex,limitMaxNumOfItems,pageResultsMaxSize,totalResultsCount )
  {
     ...
     $$('#' + inventoryListContainerId +' .sortheader' ).each(function(ele) {
       if( $s(ele) )
       {
         Event.observe( ele, 'click', inventoryList.setActiveHeader.bindAsEventListener( this, ele ) );
         Event.observe( ele, 'click', inventoryList.ajaxPostOnClick.bindAsEventListener( this, inventoryListContainerId ) );
         listenerElements.push( ele );
       }
     });
...
// Focus the active header
inventoryList.doFocusOnLoad = function( headerId )
{
  var shouldFocus = sessionStorage.getItem( headerId );
...
// Sets the active header that would get the focus after load
inventoryList.setActiveHeader = function( event, selectedColumn )
{
  sessionStorage.setItem( selectedColumn.getAttribute( 'id' ), "true" );
};
